...
- Randombot crash tracing: logs show duplicate `Item::AddToUpdateQueueOf` for same `Item*` before `EquipItem`, then only one `RemoveFromUpdateQueueOf` on destroy; stale pointer remains in `m_itemUpdateQueue` and later crashes in `_SaveInventory`. Added debug-only duplicate detection with stacktrace in `Item::AddToUpdateQueueOf` to identify who resets `uQueuePos` and re-adds.
- Added debug-only warning in `Item::SetState` when `ITEM_UNCHANGED` is set while the item is still present in the owner update queue; logs stacktrace and queue index to trace who resets `uQueuePos` without clearing the queue.
- Symbolized duplicate AddToUpdateQueueOf stack trace: `Item::SetState` -> `SetItemRandomProperties` -> `Item::CreateItem` -> `Player::EquipNewItem` -> `EquipItemToSlot` (`modules/mod-playerbots/src/RandomPlayerbotMgr.cpp:112`) -> `RunGearUpgradePass` -> `HandleBotLevelChange`. Indicates duplicate adds happen during gear upgrade while stale queue entry still exists. Core root cause appears to be stale update queue entries when `SetState(ITEM_UNCHANGED)` clears `uQueuePos` while item remains queued, so later deletions don't clear the queue slot.
- TDD exception: no existing core/unit test harness found for item update queue behavior in `src/test`; proceeding without a test per Brett’s approval.
- Item update queue fix applied: AddToUpdateQueueOf now reuses existing slots (sets `uQueuePos`, returns) with trimmed debug log; RemoveFromUpdateQueueOf repairs stale slots when `uQueuePos` is -1 and clears mismatched-owner entries; `_SaveInventory` queue-entry debug spam removed. Latest debug run showed no `SaveInventory invalid position` logs, duplicates reused slots, stale entries repaired (87 hits), and no crash; still proceeding without automated test (TDD exception).
